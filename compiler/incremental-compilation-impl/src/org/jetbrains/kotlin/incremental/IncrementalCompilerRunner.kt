...
>(
    ...
    private fun compileIncrementally(
        ...
        while (dirtySources.any() || runWithNoDirtyKotlinSources(caches)) {
            ...
            with (dirtySources) {
                clear()
                addAll(mapMemberNamesToAffectedFiles(caches.lookupCache, dirtyLookupSymbols, reporter, excludes = compiledInThisIterationSet))
                addAll(mapClassesFqNamesToAffectedFiles(caches.lookupCache, listOf(caches.platformCache), dirtyClassFqNames, reporter, excludes = compiledInThisIterationSet))
            }
